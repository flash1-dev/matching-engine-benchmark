...
| yashkukrecha ‡ | C++ | as shipped | 6.26 (normal) | — | two priority_queues + timestamp FIFO tiebreak (clean); author: incoming at Jump Trading |
...
| mercury | C++ | as shipped | 5.41 (normal) | 3.2M/s | abseil b-tree |
...
| daniele ‡ | C++ | with fix | 5.36 (normal) | — | fill-report reads a maker freed in the same fill (matching is correct); [#1](https://github.com/Daniele122898/Trading-Engine/issues/1); author: an Optiver engineer |
| johannestampere | C++ | with fix | 5.31 | — | get_best_price() returns 0 for empty book (latent; all calls guarded today) |
| ghosh (677★) ‡ | C++ | with fix | 5.05 (swing-25) | — | MIT; flat 256-slot price index shared by both sides had no collision handling → cross-side bucket merge crash; + a 1,048,576 order-id cap; both fixed; author: a low-latency trading-systems developer and Packt author; [#9](https://github.com/PacktPublishing/Building-Low-Latency-Applications-with-CPP/issues/9) |
| bozoslav | C++ | as shipped | 5.01 | — | per-side price array + slab pool + id index, native IOC/FOK/modify |
| microexchange (62★) | C++ | as shipped | 4.92 (swing-25) | 2.24M/s | array + bitmap |
| hroptatyr/clob ‡ | C | as shipped | 4.73 (normal) | ~6M/s | b+tree CLOB, `_Decimal64` (no patch); author: a Data Manager at GA Financial Solutions GmbH (GA Group / GA Asset Management, systematic funds) |
...
| ranjan2829 (131★) ‡ | C++ | with fix | 4.07 | — | 4 memory-safety defects fix [#3](https://github.com/ranjan2829/High-Frequency-Trading-Exchange-Engine/issues/3); author: an ex-Baven (GigaBrain) trading-agents engineer and former MaticAlgos quant intern |
...
| faulaire ‡ | C++ | as shipped | 3.62 | — | Boost.MultiIndex: hashed id + ordered price; author: a software engineer at One Eleven Capital (Paris quant hedge fund) |
| nanobook | Rust | as shipped | 3.52 | — | MIT; clean |
| mh2rashi | C++ | with fix | 3.47 (swing-25) | ~23k/s | 1-line `deleteOrder` list-corruption/crash fix [#4](https://github.com/mh2rashi/Trading-Engine/issues/4) |
| Tzadiko (307★) ‡ | C++ | with fix | 3.39 (flash-crash) | — | IOC self-deadlock; two-site lock-wrapper fix [#11](https://github.com/Tzadiko/Orderbook/issues/11) + [#12](https://github.com/Tzadiko/Orderbook/issues/12); author: YouTube's 'Coding Jesus' (sells online coding lectures, getcracked.io) — an ex-quant developer at an undisclosed proprietary trading firm |
...
| robaho | C++ | with fix | 1.90 (swing-25) | 10–22 M/s | execute at the resting (maker) price, not the aggressor's limit [#2](https://github.com/robaho/cpp_orderbook/issues/2); fixed upstream — `RESOLVED_FINDINGS.md` (pin stays pre-fix) |
...
| stocksharp (10236★) ‡ | C# | with fix | 1.64 (swing-25) | — | same-price orders could match out of arrival order (Dictionary enumeration replaces FIFO); conforms with the documented one-line fix; author: StockSharp / trading-tech vendor; [#681](https://github.com/StockSharp/StockSharp/issues/681); fixed upstream — `RESOLVED_FINDINGS.md` (pin stays pre-fix) |
...
| sadhbh | C++ | with fix | 1.28 (static) | — | C++20 coroutines; exact-touch crossing + empty-book deref guard [#6](https://github.com/sadhbh-c0d3/cpp20-orderbook/issues/6); fixed upstream — `RESOLVED_FINDINGS.md` (pin stays pre-fix) |
...
| vdt ‡ | JavaScript | with fix | 0.71 | — | MIT; assert never imported → ReferenceError on side guard; BitFloor's own exchange engine (US Bitcoin exchange, closed 2013) |
...
| gotrader (513★) | Go | with fix | 0.56 (swing-25) | 400k quote/s (net) | reject a modify of a fully-filled order (don't swallow-ack) [#23](https://github.com/robaho/go-trader/issues/23); fixed upstream — `RESOLVED_FINDINGS.md` (pin stays pre-fix) |
...
| circus | C# | with fix | 0.39 (swing-25) | — | reusing a completed order id crashed; partial-fill residual mishandled; fixed; [#1](https://github.com/seanoflynn/circus/issues/1); fixed upstream — `RESOLVED_FINDINGS.md` (pin stays pre-fix) |
...
| viabtc (2784★) ‡ | C | as shipped | 0.29 | — | skiplist + dict; ViaBTC's own exchange engine (official org repo) — author: the founder/CEO of ViaBTC, founder of crypto exchange CoinEx |
| omx | C | with fix | 0.29 | — | no-license; skiplist_release leaks node header+struct [#2](https://github.com/0xae/omx-engine/issues/2) |
| joaquinbejar ‡ | Rust | with fix | 0.29 (static) | — | Book::replace crossing modify never re-matches → rests a crossed book [#59](https://github.com/joaquinbejar/hft-clob-core/issues/59); author: a quant developer at Capital Delta; fixed upstream — `RESOLVED_FINDINGS.md` (pin stays pre-fix) |
...
| silue ‡ | Python | with fix | 0.26 | — | no-license; get_pnl Decimal×None → crash on first cross [#1](https://github.com/silue-dev/limit-order-book-market-making/issues/1); author: an ex-Kairon Labs quantitative-research intern (crypto market maker); fixed upstream — `RESOLVED_FINDINGS.md` (pin stays pre-fix) |
...
| mkhoshkam ‡ | Go | with fix | 0.18 | — | heap Less ignores seq → FIFO fix (adapter) [#10](https://github.com/mkhoshkam/orderbook/issues/10); author: a software engineer at Technance (trading-infrastructure vendor) |
...
| dydx ‡ | Go | as shipped | 0.11 (swing-25) | — | de-chained v4 memclob (accessor-only); dYdX's official protocol org repo (v4-chain) — author: a dYdX founder |
...
| vega ‡ | Go | as shipped | 0.08 (static) | — | accessor-only; Vega Protocol's official org repo — author: a Vega Protocol founder (designed a London Stock Exchange matcher) |
...
| rabbittrix | Rust | with fix | 0.02 | — | bid sort-key + cancel-stub fix [#1](https://github.com/rabbittrix/Ultra-Low-Latency-FX-eTrading-Platform/issues/1); fixed upstream — `RESOLVED_FINDINGS.md` (pin stays pre-fix) |
| rhodey ‡ | JavaScript | as shipped | ~0.02–0.12 (swings) | — | MIT; clean; author: an engineer at Velo Data (crypto-derivatives data & trading platform) |
...
| darkpool ‡ | C++ | with fix | very slow (static) | — | report the execution at the maker price [#1](https://github.com/dendisuhubdy/dark_pool/issues/1); author: founder of crypto exchange Bitwyre and of HFT market maker Systematic Trading Group; matcher core vendored from QuickFIX `examples/ordermatch` |
...
| khrapovs ‡ | Python | with fix | very slow (pure Python) | — | MIT; orders_by_expiration not pruned on fill [#25](https://github.com/khrapovs/OrderBookMatchingEngine/issues/25); author: a senior ML engineer at ING (bank); fixed upstream — `RESOLVED_FINDINGS.md` (pin stays pre-fix) |
