...
- `Exchange::submitOrder(Order)` — async submit through an SPSC ring to a
  per-shard worker thread.
- `Exchange::cancelOrder(symbolId, orderId)` — async, `void`, no
  success/failure return.
- `OrderBook::addOrder/cancelOrder` — synchronous primitives the worker uses.
...
- Synthesises **OrderAck / CancelAck / ModifyAck / CancelReject / ModifyReject**
  above the engine. A shadow map `oid -> {price, side, remaining, alive}` is
  updated from the matching strategy's returned fill list.
