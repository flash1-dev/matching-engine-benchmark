/*
 * jlob_adapter.cpp — matching_engine_api.h backed by jLOB.
 *
 * jLOB (https://github.com/eliquinox/jLOB) is a Java L3 limit order book:
 * price-sorted fastutil Long2ObjectRBTreeMaps of `Limit` price levels (each a
 * FIFO ArrayList of `Placement`s) with a UUID->Placement index; matching
 * (state.LimitOrderBook.place) crosses an incoming order against the best contra
 * levels price-time then rests the residual. This adapter embeds a JVM via JNI
 * and drives a jLOB LimitOrderBook through the HarnessJlob Java helper. Built as
 * a shared library by build.sh.
 *
 * No jLOB source is patched — all harness glue (per-message API, report
 * derivation, modify handling, IOC) lives in this adapter and the helper.
 *
 * The adapter emits the report stream itself. HarnessJlob registers itself as
 * the book's LimitOrderBookListener and writes one fill record per onMatch into
 * an adapter-owned staging buffer; this adapter turns each into a Trade report
 * and adds the OrderAck / CancelAck / ModifyAck (and CancelReject / ModifyReject)
 * reports, pushing all of them into the harness report transport. Modify is
 * cancel + reinsert, performed inside the helper; IOC residuals are cancelled via
 * the engine's own cancel and reported as a CancelAck.
 *
 * Threading: every engine_* call runs on the harness matcher thread — the same
 * thread that creates the JVM — so one cached JNIEnv is valid throughout. jLOB
 * matches synchronously on that thread, so engine_flush() is a no-op. The JVM's
 * own service threads (GC, JIT) are created during engine_init; an engine is free
 * to use threads, so they are not policed.
 *
 * This file follows the same JNI-bridge shape as the other embedded-JVM
 * adapters; the only engine-specific differences are the helper's method
 * signatures and the cancel echo (onCancel returns 1/0 and stages the cancelled
 * order's price @8 + side @16 of record 0).
 */
...
/* Full path to libjvm.so, baked in by build.sh. Overridable at run time with the
 * ME_JVM_LIB environment variable. */
#ifndef ME_JVM_LIB
...
jobject g_engine = nullptr;      /* HarnessJlob instance (global ref) */
...
/* Adapter-owned staging buffer: HarnessJlob writes one record per fill, this
 ...
 * field (@8) and its side in record 0 @16. */
struct StageTrade {
    uint64_t seq;
    int64_t  price;
    uint32_t qty;
...
[[noreturn]] void fatal(const char* msg) {
    std::fprintf(stderr, "jlob_adapter: %s\n", msg);
...
void check_exception(const char* where) {
    if (g_env->ExceptionCheck()) {
        std::fprintf(stderr, "jlob_adapter: Java exception in %s\n", where);
...
/* Resolve the jLOB classpath: ./jlob.classpath (written by build.sh), else the
 * ME_JLOB_CLASSPATH env var. */
std::string load_classpath() {
    if (FILE* f = std::fopen("jlob.classpath", "rb")) {
        std::string cp;
        char buf[8192];
        size_t n;
    ...
    if (const char* env = std::getenv("ME_JLOB_CLASSPATH")) return env;
    fatal("no classpath: expected ./jlob.classpath or $ME_JLOB_CLASSPATH "
          "(run: build.sh)");
}
...
void engine_init(uint64_t /*seed*/, const me_transport_t* transport,
     ...
     * the timed window (warmup deliberately redirects to a scratch buffer and
     * leaves g_stage untouched). */
    ...
    jclass cls = g_env->FindClass("HarnessJlob");
    if (!cls) fatal("HarnessJlob not found on the classpath");
    ...
    check_exception("HarnessJlob.<init>");
    if (!obj) fatal("NewObject(HarnessJlob) failed");
    ...
    check_exception("HarnessJlob.warmup");
}
...
/* jLOB matches synchronously on the calling thread — nothing pending. */
...
void engine_on_cancel(const cancel_t* c) {
    /* onCancel removes via jLOB's own cancel(Cancellation): returns 1 if the
     * order was resting and is now cancelled (its price staged in g_stage[0].price
     * @8, its side in g_stage[0] @16), or 0 when the order is not resting. */
    jint resting = g_env->CallIntMethod(g_engine, m_onCancel,
                                        static_cast<jlong>(c->order_id));
    check_exception("onCancel");
    if (resting) {
        uint8_t side = static_cast<uint8_t>(g_stage[0].qty);   // side echoed @16
        emit_ack(ME_CANCEL_ACK, c->sequence_number, c->order_id,
                 side, g_stage[0].price, 0);
    } else {
