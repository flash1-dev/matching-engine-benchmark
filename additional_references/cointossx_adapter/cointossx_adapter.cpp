/*
 * cointossx_adapter.cpp — matching_engine_api.h backed by CoinTossX's matching
 * core.
 *
 * CoinTossX (https://github.com/dharmeshsing/CoinTossX) is a Java JSE exchange
 * on Aeron/Agrona. This adapter does NOT run the exchange: it embeds a JVM via
 * JNI and drives CoinTossX's separable in-process matcher
 * (orderBook.OrderBook + crossing.tradingSessions.ContinuousTradingProcessor)
 * through the HarnessCoinTossX helper — no Aeron, UDP, or disruptor in the loop.
 *
 * One minimal engine patch (a per-fill observation sink) is applied in build.sh;
 * all harness glue (per-message API, report derivation, modify = cancel +
 * reinsert) lives in this adapter and the helper. See the PATCHES note in
 * build.sh / HarnessCoinTossX.java.
 *
 * The adapter emits the report stream itself. HarnessCoinTossX writes the trades
 * the engine's matcher produces into an adapter-owned staging buffer; this
 * adapter turns each into a Trade report and adds the OrderAck / CancelAck /
 * ModifyAck (and CancelReject / ModifyReject) reports, pushing all into the
 * harness report transport.
 *
 ...
 * CoinTossX's continuous-trading matcher runs synchronously on that thread, so
 * engine_flush() is a no-op. The JVM's service threads (GC, JIT) are created
 * during engine_init; an engine is free to use threads, so they are not policed.
 *
 * Modelled on adapters/exchange_core_adapter.cpp (the JNI reference).
 */
...
jobject g_engine = nullptr;      /* HarnessCoinTossX instance (global ref) */
...
/* Adapter-owned staging buffer: HarnessCoinTossX writes one record per fill,
...
[[noreturn]] void fatal(const char* msg) {
    std::fprintf(stderr, "cointossx_adapter: %s\n", msg);
...
void check_exception(const char* where) {
    if (g_env->ExceptionCheck()) {
        std::fprintf(stderr, "cointossx_adapter: Java exception in %s\n", where);
...
/* Resolve the classpath: ./cointossx.classpath (written by build.sh), else the
 * ME_CTX_CLASSPATH env var. */
std::string load_classpath() {
    if (FILE* f = std::fopen("cointossx.classpath", "rb")) {
        std::string cp;
        char buf[8192];
        size_t n;
    ...
    if (const char* env = std::getenv("ME_CTX_CLASSPATH")) return env;
    fatal("no classpath: expected ./cointossx.classpath or $ME_CTX_CLASSPATH "
          "(run: ./build.sh)");
}
...
void engine_init(uint64_t /*seed*/, const me_transport_t* transport,
    ...
    /* SerialGC + a fixed pre-touched 2 GiB heap removes heap-resize and page-
     * fault noise from the measured pass. The off-heap order nodes
     * (sun.misc.Unsafe) live outside this heap. */
    ...
    jclass cls = g_env->FindClass("HarnessCoinTossX");
    if (!cls) fatal("HarnessCoinTossX not found on the classpath");
    ...
    check_exception("HarnessCoinTossX.<init>");
    if (!obj) fatal("NewObject(HarnessCoinTossX) failed");
    ...
    check_exception("HarnessCoinTossX.warmup");
}
...
/* CoinTossX matches synchronously on the calling thread — nothing pending. */
...
void engine_on_cancel(const cancel_t* c) {
    /* onCancel removes via the engine's own off-heap level + B+Tree, returning
     * the cancelled order's side as 1(bid)/2(ask) with its price staged in
     * g_stage[0].price, or 0 when the order is not resting. */
