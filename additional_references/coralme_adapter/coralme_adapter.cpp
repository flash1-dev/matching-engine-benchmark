/*
 * coralme_adapter.cpp — matching_engine_api.h backed by CoralME.
 *
 * CoralME (https://github.com/coralblocks/CoralME) is a garbage-free Java
 * matching engine (price-ordered doubly-linked price-level list + per-level
 * FIFO order list + LongMap id index, all pooled). This adapter embeds a JVM
 * via JNI and drives a CoralME OrderBook through the HarnessCoralMe Java helper.
 * Built as a shared library by build.sh.
 *
 * No CoralME source is patched — all harness glue (per-message API, report
 * derivation, modify handling) lives in this adapter and the helper.
 *
 * The adapter emits the report stream itself. HarnessCoralMe registers itself
 * as the book's OrderBookListener and writes one fill record per MAKER-side
 * execution into an adapter-owned staging buffer; this adapter turns each into
 * a Trade report and adds the OrderAck / CancelAck / ModifyAck (and
 * CancelReject / ModifyReject) reports, pushing all of them into the harness
 * report transport. Modify is cancel + reinsert, performed inside the helper.
 *
 ...
 * CoralME matches synchronously on that thread, so engine_flush() is a no-op.
 * The JVM's own service threads (GC, JIT) are created during engine_init; an
 * engine is free to use threads, so they are not policed.
 *
 * This file mirrors adapters/exchange_core_adapter.cpp; the only engine-specific
 * differences are the helper's method signatures (onCancel returns 1/0 and the
 * cancelled side comes from lastCancelSide(); depthAt walks the level list).
 */
...
jobject g_engine = nullptr;      /* HarnessCoralMe instance (global ref) */
...
/* Adapter-owned staging buffer: HarnessCoralMe writes one record per fill, this
 ...
 * this side drains it to the transport. */
...
[[noreturn]] void fatal(const char* msg) {
    std::fprintf(stderr, "coralme_adapter: %s\n", msg);
...
void check_exception(const char* where) {
    if (g_env->ExceptionCheck()) {
        std::fprintf(stderr, "coralme_adapter: Java exception in %s\n", where);
...
/* Resolve the CoralME classpath: the file ./coralme.classpath (written by
 * build.sh), else the ME_CORALME_CLASSPATH env var. */
std::string load_classpath() {
    if (FILE* f = std::fopen("coralme.classpath", "rb")) {
        std::string cp;
        char buf[4096];
        size_t n;
    ...
    if (const char* env = std::getenv("ME_CORALME_CLASSPATH")) return env;
    fatal("no classpath: expected ./coralme.classpath or $ME_CORALME_CLASSPATH "
          "(run: build.sh)");
}
...
void engine_init(uint64_t /*seed*/, const me_transport_t* transport,
    ...
    jclass cls = g_env->FindClass("HarnessCoralMe");
    if (!cls) fatal("HarnessCoralMe not found on the classpath");
    ...
    check_exception("HarnessCoralMe.<init>");
    if (!obj) fatal("NewObject(HarnessCoralMe) failed");
    ...
    check_exception("HarnessCoralMe.warmup");
}
...
/* CoralME matches synchronously on the calling thread — nothing pending. */
...
void engine_on_cancel(const cancel_t* c) {
    /* onCancel answers from CoralME's own id-keyed getOrder: returns 1 if the
     * order was resting and is now cancelled (its price in g_stage[0].price, its
     * side bit via lastCancelSide), or 0 when the order is not resting. No
