...
reflect their authors' own goals. 

No number or finding here ranks or judges an author's engineering quality — the harness merely reports what each pinned commit does. Affiliations are as the authors state them publicly.
...
official vendor/org repo. Affiliations are as the authors state them publicly (GitHub profile /
bio). · **★** = repository has 50+ GitHub stars. Rows are sorted
...
Phoenix cleanly; dYdX and Vega with a read-only accessor patch) at 0.08–2.6 M/s worst-case.

> **What `accessor-only` means.** It denotes the *only* modification made to those two engines, not
> a limit on what was tested. dYdX and Vega keep their book private to their own package, so the
> harness's book-state audit has no way to read `best_bid` / `best_ask` / `depth_at`. The patch is a
> minimal, additive, side-effect-free **read-only accessor** that forwards to the engine's own
> already-correct state. **No matching, ordering, or crossing line is touched**, and the full matching path is driven through each engine's own public API.

The **4 measured through
