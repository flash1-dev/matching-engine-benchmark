...
| CppTrader ‡ | C++ | — | 7.26 (normal) | conforms | — on canonical; a `ModifyOrder` crash off the canonical path is fixed upstream; author: Head of C++ Development at Finstek (an FX/CFD trading-platform vendor) | resolved ([#42](https://github.com/chronoxor/CppTrader/issues/42)) |
...
| llc993 | Rust | — | 7.14 (swing-25) | conforms | — (BTreeMap + slab pool + intrusive time-queue) | — |
...
| yashkukrecha ‡ | C++ | none | 6.26 (normal) | conforms | — (clean; two `priority_queue`s with a timestamp FIFO tiebreak); author: incoming at Jump Trading | — |
...
| mercury | C++ | — | 5.41 (normal) | conforms | — (abseil b-tree) | — |
...
| microexchange | C++ | — | 4.92 (swing-25) | conforms | — (array + bitmap) | — |
| hroptatyr/clob ‡ | C | — | 4.73 (normal) | conforms | — (b+tree CLOB, `_Decimal64`, no patch); author: a Data Manager at GA Financial Solutions GmbH (GA Group / GA Asset Management, systematic funds) | — |
...
| ranjan2829 ‡ | C++ | — | 4.07 | conforms | 4 memory-safety defects (null-deref, OOB ring, wedging pool, uninitialised index arrays); author: an ex-Baven (GigaBrain) trading-agents engineer and former MaticAlgos quant intern | [#3](https://github.com/ranjan2829/High-Frequency-Trading-Exchange-Engine/issues/3) |
...
| faulaire ‡ | C++ | — | 3.62 | conforms | — (Boost.MultiIndex: hashed id + ordered price; conforms as shipped); author: a software engineer at One Eleven Capital (Paris quant hedge fund) | — |
| nanobook | Rust | MIT | 3.52 | conforms | — (clean) | — |
| mh2rashi | C++ | — | 3.47 (swing-25) | conforms | `deleteOrder` guard nulls a 2-order level's survivor → list corruption/crash on all five; 1-line fix → VALID ×5 | [#4](https://github.com/mh2rashi/Trading-Engine/issues/4) |
| Tzadiko ‡ | C++ | — | 3.39 (flash-crash) | conforms | self-deadlocks on a partially-filled IOC; two-site `CancelOrderInternal` patch → VALID ×5 (+ teardown lost-wakeup, filed separately); author: YouTube's 'Coding Jesus' (sells online coding lectures, getcracked.io) — an ex-quant developer at an undisclosed proprietary trading firm | [#11](https://github.com/Tzadiko/Orderbook/issues/11) + [#12](https://github.com/Tzadiko/Orderbook/issues/12) |
...
| lanpishu | C++ | — | 2.66 (static) | non-conforming | broken RB-tree delete-fixup → wrong best price/counterparty + under-match (can hang); depth double-decrement → phantom levels; fixed upstream — `RESOLVED_FINDINGS.md` (the verdict describes the pinned commit) | [#2](https://github.com/lanpishu6300/crypto-exchange/issues/2) |
...
| robaho | C++ | — | 1.90 (swing-25) | conforms | trade priced at the aggressor's limit, not the maker's (price-priority); `static` passes — fix verified, VALID ×5 across 100 seeds; fixed upstream — `RESOLVED_FINDINGS.md` (pin stays pre-fix) | [#2](https://github.com/robaho/cpp_orderbook/issues/2) |
...
| piquette ‡ | Go | — | 1.28 (flash-crash) | non-conforming | bid/ask maps never populated → every cancel rejected; duplicate-price stranding; best-ask uses `>` not `<`; author: a financial services engineer at Watershed Technologies (equity-options ATS) | [#2](https://github.com/piquette/orderbook/issues/2) |
| sadhbh | C++ | — | 1.28 (static) | conforms | order at the exact touch never crosses; opposite-side guard dereferences an empty book (UB) — fixed; fixed upstream — `RESOLVED_FINDINGS.md` (pin stays pre-fix) | [#6](https://github.com/sadhbh-c0d3/cpp20-orderbook/issues/6) |
...
| vdt ‡ | JavaScript | MIT | 0.71 | conforms | `assert` is never imported → `ReferenceError` on the side guard; BitFloor's own exchange engine (US Bitcoin exchange, closed 2013), mirrored; the repo owner is an uninvolved re-hoster | — |
...
| gotrader | Go | — | 0.56 (swing-25) | conforms | a modify of a fully-filled order is swallowed (acked), not rejected — `ModifyOrder` swallow (conformance gate) — fix verified, VALID ×5 across 100 seeds; fixed upstream — `RESOLVED_FINDINGS.md` (pin stays pre-fix) | [#23](https://github.com/robaho/go-trader/issues/23) |
...
| mercury (notayessir) | Java | MIT | 0.39 | conforms | a fully-consumed maker is never evicted from the `orders` id-index → a later cancel of that filled id is acked and NPEs | [#1](https://github.com/notayessir/mercury-match-engine/issues/1) |
| circus | C# | — | 0.39 (swing-25) | conforms | reusing a completed order id crashes; a partial-fill-then-modify silently rests short; `Process()` swaps `Price`/`TriggerPrice`; fixed; fixed upstream — `RESOLVED_FINDINGS.md` (pin stays pre-fix) | [#1](https://github.com/seanoflynn/circus/issues/1) |
...
| viabtc ‡ | C | — | 0.29 | conforms | — (skiplist + dict; conforms as shipped); ViaBTC's own exchange engine (official org repo) — author: the founder/CEO of ViaBTC, founder of crypto exchange CoinEx | — |
| joaquinbejar ‡ | Rust | MIT | 0.29 (static) | conforms | Book::replace's lose-priority path never re-matches → rests a crossed book; author: a quant developer at Capital Delta; fixed upstream — `RESOLVED_FINDINGS.md` (pin stays pre-fix) | [#59](https://github.com/joaquinbejar/hft-clob-core/issues/59) |
...
| silue ‡ | Python | none | 0.26 | conforms | `get_pnl` computes `Decimal × None` → crash on the first cross; author: an ex-Kairon Labs quantitative-research intern (crypto market maker); fixed upstream — `RESOLVED_FINDINGS.md` (pin stays pre-fix) | [#1](https://github.com/silue-dev/limit-order-book-market-making/issues/1) |
...
| lightning (754liam) | C++ | MIT | 0.22 (static) | conforms | `matchAskLimit` prices fills at the aggressor's limit, not the maker's resting bid (measured with the maker-price fix) | [#1](https://github.com/754liam/Lightning/issues/1) |
| mkhoshkam ‡ | Go | — | 0.18 | conforms | heap `Less` ignores the sequence number → FIFO at equal price not preserved (adapter patch restores it); author: a software engineer at Technance (trading-infrastructure vendor) | [#10](https://github.com/mkhoshkam/orderbook/issues/10) |
...
| dydx ‡ | Go | GPL-3.0 | 0.11 (swing-25) | conforms | — (de-chained v4 memclob, accessor-only); dYdX's official protocol org repo (v4-chain) — author: a dYdX founder | — |
...
| vega ‡ | Go | AGPL-3.0 | 0.08 (static) | conforms | — (accessor-only); Vega Protocol's official org repo — author: a Vega Protocol founder (designed a London Stock Exchange matcher) | — |
...
| baoyingwang | Java | MIT | 0.04 (static; ~1.6 typical) | conforms | a FIFO-tiebreak subtraction overflows past a 24.85-day order age; the deep-tape crash is a separate, new robustness finding | [#4](https://github.com/baoyingwang/OrderBook/issues/4) |
...
| rabbittrix | Rust | — | 0.02 | conforms | bid `binary_search_by_key` mismatches its sort key → bids unsorted, marketable sells don't cross; `cancel_order` is a stub; fixed upstream — `RESOLVED_FINDINGS.md` (pin stays pre-fix) | [#1](https://github.com/rabbittrix/Ultra-Low-Latency-FX-eTrading-Platform/issues/1) |
| rhodey ‡ | JavaScript | MIT | ~0.02–0.12 (swings) | conforms | — (clean); author: an engineer at Velo Data (crypto-derivatives data & trading platform) | — |
...
| lll ‡ | C++ | none | infeasible (static) | non-conforming | infeasible at 2 M — stalls at the 600 s watchdog on deep static 2 M (seed 11930289) at both the uint16 and a widened build (quiet-box A/B); `modify_order_in_map` no-op is a separate latent issue; author: a Citadel Securities engineer (HFT systems) — engine written as a student, predates employment | [Suggestion] [#1](https://github.com/northwesternfintech/low-latency-league/issues/1) |
| prystupa ‡ | Scala | none | infeasible (static) | non-conforming | infeasible at 2 M — a deep stressful-static book stalls past the 600 s watchdog (its 0.02 M/s was seed-23 static); the latent `FastList.removeInto` tail-drop is filed separately; author: an ex-Lab49 engineer (15+ years building trading systems); the latent finding is fixed upstream — `RESOLVED_FINDINGS.md` (the infeasible verdict is unchanged) | [#6](https://github.com/prystupa/scala-cucumber-matching-engine/issues/6) |
...
| darkpool ‡ | C++ | — | very slow (static) | conforms | aggressor pricing in the execution-price report — fix verified, VALID ×5 across 100 seeds; author: founder of crypto exchange Bitwyre and of HFT market maker Systematic Trading Group; matcher core vendored from QuickFIX `examples/ordermatch` | [#1](https://github.com/dendisuhubdy/dark_pool/issues/1) |
| fasenderos | TypeScript | — | very slow (static) | conforms | — (libnode embed; very slow in its weakest scenario) | — |
| khrapovs ‡ | Python | MIT | very slow (pure Python) | conforms | `orders_by_expiration` not pruned on fill; author: a senior ML engineer at ING (bank); fixed upstream — `RESOLVED_FINDINGS.md` (pin stays pre-fix) | [#25](https://github.com/khrapovs/OrderBookMatchingEngine/issues/25) |
...
| big_order_book | JavaScript | GPL-3.0 | crash | non-conforming | a fully-filled maker is detached but never removed from `orderItemMap`, so a later cancel/modify of that id dereferences a null list (a harness crash); GPL handling = glue shipped, engine fetched at build; fixed upstream — `RESOLVED_FINDINGS.md` (the crash verdict describes the pinned commit) | [#1](https://github.com/Capitalisk/big-order-book/issues/1) |
...
| chessbr | Rust | — | infeasible (2 M) | non-conforming | sells matched the lowest resting bid, not the best — fixed, but the engine is O(n²) and cannot finish 2 M | [#4](https://github.com/chessbr/rust-exchange/issues/4) |
...
| dylanlott | Go | MIT | crash (empty book) | non-conforming | `MatchOrders` dereferences `buyOrders[0]` before checking the buy side is non-empty → a sell into an empty book panics (the underlying non-conformance — a descending sell-side sort and a wrong-side fill quantity — is left intact) | [#10](https://github.com/dylanlott/orderbook/issues/10) |
...
| hnodomar | C++ | no license | crash | non-conforming | a `Level&` bound once is freed mid-sweep by `book.erase`, then read and written through the dangling reference → heap use-after-free, corrupting later matches at that price | [#1](https://github.com/Hnodomar/Spot-Exchange/issues/1) |
...
| mattdavey ‡ | Java | — | infeasible | non-conforming | `OrderBook.placeOrderInBook` rescans the resting side on every insert — O(n)/order, O(n²) total as the book deepens; infeasible at 2 M; authors: two then-Lab49 engineers (trading-technology consultancy) | [#26](https://github.com/mattdavey/EuronextClone/issues/26) |
...
| peatio ‡ | Ruby | — | infeasible | non-conforming | byte-identical on every completed cell; the Ruby rbtree engine exceeds the watchdog; the open-source Peatio exchange engine — principal author an ex-core developer at Yunbi (closed crypto exchange) | — |
| pyobsim ‡ | Python | — | — | non-conforming | `Side.remove` deletes only the head order and `Book.__match` mutates the level while iterating it; author: a trading-systems engineer at tplus (decentralized prime exchange); fixed upstream — `RESOLVED_FINDINGS.md` (the verdict describes the pinned commit) | [#2](https://github.com/jmcph4/PyOBSim/issues/2) |
...
| thelilypad ‡ | Python | — | infeasible | non-conforming | same-price orders can fill out of arrival order (`Order.__lt__` has no tiebreak, so `heapq` doesn't preserve FIFO); diverges + infeasible at 2 M; author: a quantitative options trader/researcher — creator of the NOPE indicator (Salience Capital) | [#10](https://github.com/thelilypad/orderbook_simulator/issues/10) |
...
| wailo | C++ | — | crash (static) | non-conforming | reflexive `operator>` (invalid `std::*_heap` comparator, UB) fixed, but no time priority + silent intake-queue drops remain | [#1](https://github.com/wailo/orderbook-matching-engine/issues/1) |
...
| zzsun777 ‡ | C++ | — | infeasible | non-conforming | byte-identical on every completed cell; O(n) by-owner find exceeds the watchdog on its worst scenario; mirror — authored by the founder of trading-software vendor Coreware Ltd (llfix FIX engine), at Thomson Reuters when the engine was written (2016); the repo owner is an uninvolved re-hoster | — |
...
| pylob ‡ | Python | — | infeasible (static) | non-conforming | SQLite-backed; static times out at 2 M; author: a JPMorgan engineer; the two bugs are fixed upstream — `RESOLVED_FINDINGS.md` (the infeasible verdict is unchanged) | [#8](https://github.com/DrAshBooth/PyLOB/issues/8) |
...
| konqr ‡ | Python | — | — | non-conforming | Hawkes-process simulator — not a price-time CLOB in the form the benchmark needs; author: a Tower Research Capital quant (ex-JPMorgan Equities Electronic Trading) | [#19](https://github.com/konqr/lobSimulations/issues/19) |
