/*
 * harness.h — internal interfaces shared across the harness translation units.
 * Not part of the public engine ABI (that is api/matching_engine_api.h).
 */
...
/* One workload message, decoded from a 40-byte orders_*.bin record. */
struct WorkloadMsg {
...
/* Load an orders_*.bin workload file. Returns false on a missing/corrupt file. */
